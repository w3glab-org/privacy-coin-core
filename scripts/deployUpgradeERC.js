...
async function main() {
  ...
  const proxy = await ethers.getContractAt('ERCPool', proxyAddress)
  ...
  const tokenAddress = await proxy.token()

  console.log(
    `Using existing immutables: verifier2=${verifier2Address}, levels=${levels}, hasher=${hasherAddress}, token=${tokenAddress}`,
  )

  const ERCPoolV2 = await ethers.getContractFactory('ERCPool')

  const newImpl = await upgrades.prepareUpgrade(proxyAddress, ERCPoolV2, {
    kind: 'uups',
    constructorArgs: [verifier2Address, levels, hasherAddress, tokenAddress],
